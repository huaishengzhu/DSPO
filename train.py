...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        if args.unet_path is None:
            ...
            ref_unet = UNet2DConditionModel.from_pretrained(
            args.unet_init if args.unet_init else args.unet_path,
            subfolder="unet", revision=args.revision
        )
    if args.unet_init:
    ...
    if args.unet_path is None:
        ...
        unet = UNet2DConditionModel.from_pretrained(
            args.unet_init if args.unet_init else args.unet_path, subfolder="unet", revision=args.revision
        )
    ...
    if args.train_method == 'dspo': ref_unet.requires_grad_(False)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            if len(models) > 1:
                assert args.train_method == 'dspo' # 2nd model is just ref_unet in DPO case
    ...
    if 'pickapic' in args.dataset_name or (args.train_method == 'dspo'):
        pass
